...
def dedup_gfw_events() -> pd.DataFrame:
    ...
    df = pd.read_parquet(INPUT_DIR / "gfw_events_flat.parquet")
    before = len(df)
...
def dedup_sar_presence() -> pd.DataFrame:
    ...
    df = pd.read_parquet(INPUT_DIR / "sar_presence_flat.parquet")
    before = len(df)
    
    # Drop rows without MMSI (grid-only, can't track vessels)
    no_mmsi = (df["mmsi"] == "").sum()
...
def dedup_fishing_effort() -> pd.DataFrame:
    ...
    df = pd.read_parquet(INPUT_DIR / "fishing_effort_flat.parquet")
    before = len(df)
    ...
    if dupes > 0:
        # Sum fishing_hours for duplicate keys
...
def dedup_zenodo_effort() -> pd.DataFrame:
    ...
    pf = pq.ParquetFile(INPUT_DIR / "zenodo_effort_flat.parquet")
...
def dedup_vessel_registry() -> pd.DataFrame:
    ...
    df = pd.read_parquet(INPUT_DIR / "vessel_registry.parquet")
...
def run_step_2_1():
    ...
    print("=" * 60)
    print("STEP 2.1 — DEDUPLICATION")
    print("=" * 60)
    
    # 2.1a: GFW Events
    df_events = dedup_gfw_events()
    df_events.to_parquet(OUTPUT_DIR / "gfw_events_dedup.parquet", index=False)
    print(f"  ✅ gfw_events_dedup.parquet ({len(df_events):,} rows)")
    del df_events; gc.collect()
    
    # 2.1b: SAR Presence
    df_sar = dedup_sar_presence()
    df_sar.to_parquet(OUTPUT_DIR / "sar_presence_dedup.parquet", index=False)
    print(f"  ✅ sar_presence_dedup.parquet ({len(df_sar):,} rows)")
    del df_sar; gc.collect()
    
    # 2.1c: Fishing Effort
    df_effort = dedup_fishing_effort()
    df_effort.to_parquet(OUTPUT_DIR / "fishing_effort_dedup.parquet", index=False)
    print(f"  ✅ fishing_effort_dedup.parquet ({len(df_effort):,} rows)")
    del df_effort; gc.collect()
    
    # 2.1d: Zenodo Effort (memory-efficient)
    df_zenodo = dedup_zenodo_effort()
    df_zenodo.to_parquet(OUTPUT_DIR / "zenodo_effort_dedup.parquet", index=False)
    print(f"  ✅ zenodo_effort_dedup.parquet ({len(df_zenodo):,} rows)")
    del df_zenodo; gc.collect()
    
    # 2.1e: Vessel Registry
    df_vessels = dedup_vessel_registry()
    df_vessels.to_parquet(OUTPUT_DIR / "vessel_registry_dedup.parquet", index=False)
    print(f"  ✅ vessel_registry_dedup.parquet ({len(df_vessels):,} rows)")
