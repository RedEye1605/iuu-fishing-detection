...
def run_step_3_4():
    ...
    print("=" * 60)
    print("STEP 3.4: BEHAVIORAL FEATURES (PER VESSEL)")
    print("=" * 60)
    ...
    df = pd.read_parquet(INPUT / "gfw_events_enriched.parquet", columns=cols)
    print(f"Loaded: {len(df):,} events from {df['mmsi'].nunique():,} vessels")
    
    # Sort by vessel + time for rolling computations
    df = df.sort_values(["mmsi", "start_time"]).reset_index(drop=True)
    
    # ===== PER-VESSEL AGGREGATES (overall) =====
    print("\nComputing per-vessel overall stats...")
    ...
    spatial = df.groupby("mmsi").agg(
        ...
        lon_min=("lon", "min"),
        lon_max=("lon", "max"),
        unique_grid_cells=("grid_lat", lambda x: len(set(zip(
            df.loc[x.index, "grid_lat"], df.loc[x.index, "grid_lon"])))),
    ).reset_index()
    
    # Spatial range (km) - haversine approximation
    spatial["spatial_range_km"] = np.sqrt(
        (spatial["lat_max"] - spatial["lat_min"])**2 + 
        (spatial["lon_max"] - spatial["lon_min"])**2
    ) * 111
    vessel_stats = vessel_stats.merge(spatial[["mmsi", "avg_distance_shore", "max_distance_shore", 
                                                 "spatial_range_km", "unique_grid_cells"]], 
                                       on="mmsi", how="left")
    ...
    # ===== VALIDATION =====
    print(f"\n--- Validation ---")
    print(f"Shape: {len(vessel_stats):,} vessels × {len(vessel_stats.columns)} cols")
    print(f"\nEvent count distribution:")
    print(vessel_stats["total_events"].describe().to_string())
    print(f"\nFishing count distribution:")
    print(vessel_stats["fishing_count"].describe().to_string())
    print(f"\nEncounter rate stats:")
    print(vessel_stats["encounter_rate"].describe().to_string())
    print(f"\nLoitering rate stats:")
    print(vessel_stats["loitering_rate"].describe().to_string())
    
    # Save
    out = OUTPUT / "vessel_behavioral_features.parquet"
