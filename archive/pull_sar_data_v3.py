...
def try_with_url_construction():
    """Try constructing URL with datasets query param properly."""
    log_message("Attempting with manual URL construction...")

    # Construct the full URL with query parameters
    base_url = "https://gateway.api.globalfishingwatch.org/v3/4wings/report"
    query_params = "datasets=public-global-sar-presence:latest&date-range=2020-01-01,2025-04-30&format=JSON&spatial-resolution=LOW&temporal-resolution=MONTHLY"
    ...
    try:
        response = requests.post(full_url, headers=headers, json=body, timeout=300)
        log_message(f"URL: {full_url[:200]}...")
        response.raise_for_status()

        data = response.json()
        log_message(f"Request succeeded. Status: {response.status_code}")
...
def try_bbox_with_url():
    """Try bbox approach with manual URL."""
    log_message("Attempting bbox with manual URL construction...")
...
def main():
    ...
    data = try_with_url_construction()

    if data is None:
        data = try_bbox_with_url()
